...
| Бездивергентная инициализация карт 4 и 5 | `B_n` бралось сэмплированием в середине ребра; для непостоянного поля контурная сумма по треугольнику не равна нулю на произвольной сетке. Теперь `B_n` --- разность узловых значений `A_z`, а клеточное `B` --- RT0-проекция тех же граней | **измерено на нерегулярной сетке Netgen** (5790 треугольников): начальная `max\|div B\|` поля ОТ `8.944e-02` (старая форма) против `5.336e-15` (исправленная) --- 13 порядков. Начальная невязка потока ОТ `1.0e-17`, цилиндра `1.0e-16`; `rho_min` цилиндра сместился `0.764 -> 0.743` | на структурной сетке старая форма давала нуль случайно (слагаемые телескопируются), поэтому дефект там ненаблюдаем |
...
| Учёт энергии при RT0-реконструкции (D-009) | Клеточное `B` пересобирается из граневого каждую итерацию, магнитная энергия ячейки меняется, а `E` намеренно не корректировалась — всю разницу поглощала внутренняя энергия, и при `beta -> 0` накопление уводило её в минус | Режим `preserve_internal` (по умолчанию): `E` сдвигается на изменение магнитной энергии. Вихрь ОТ 128×128 доходит до `t=0.5` за 2012 итераций с **0** срабатываний пола; прежний режим `conservative` — 14300 срабатываний | полная энергия меняется ровно на уже логируемую `ctMagneticEnergyChangeSigned`; прежнее поведение сохранено под `ctEnergyMode: conservative` как измерение |
| Пол по давлению со счётчиком | Неположительное `p` безусловно прекращало расчёт: отказ не диагностирует, а просто останавливает | `pressureFloor > 0` поднимает `p` до пола, восстанавливая `e`, и считает события; счётчик печатается и попадает в манифест | ненулевой счётчик — часть результата, а не скрытая правка; при `pressureFloor = 0` поведение прежнее (отказ) |
