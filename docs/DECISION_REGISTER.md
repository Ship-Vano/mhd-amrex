...
| D-008 | GPU status | **решено (2026-09-13)**: CUDA-путь собран и исполнен на RTX 4090 (CUDA 12.4, GCC 13.4); `ctest --preset cuda-release` 22/22; CPU/GPU parity пройден на постоянном состоянии и Орзага–Танге на штатном допуске. Ускорение НЕ измерялось. Остаются: single-GPU benchmark, multi-GPU MPI, Nsight | internal: свободная карта для замеров | T12 |
