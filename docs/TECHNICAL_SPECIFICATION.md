...
Статус: проект ТЗ, уточнённый 2026-08-26 после аудита legacy-проекта, ВКР и
формализации дальнейшего DG-направления.
...
Исполняемая декомпозиция на задания Terra приведена в
...
1. объяснить и количественно исследовать «немонотонность» в задаче Brio–Wu;
   проверить её наличие в legacy на эквивалентной постановке; определить,
   является ли она физической структурой, паразитной осцилляцией или эффектом
   AMR, и только после диагноза проверить способы уменьшения дефекта — адаптацию
   сетки, limiter/reconstruction, CT-ЭДС и positivity fallback;
...
6. подготовить кластерные job scripts, сырые результаты, таблицы и итоговый
   научно-технический отчёт в формате сокращённой расчётно-пояснительной
   записки НИРС для МГТУ им. Н. Э. Баумана;
7. проверить гипотезу, что AMReX-вариант обеспечивает лучшее соотношение
   «ошибка — время — память»; не считать превосходство доказанным заранее;
8. сформировать проверяемые архитектурные границы для последующего добавления
   discontinuous Galerkin метода без реализации DG в текущей фазе.
...
Ближайший обязательный результат — исправленная РПЗ для научного руководителя.
Она должна отражать состояние «код запускается, полная верификация ещё не
завершена» и не выдавать планы AMR/GPU/scaling за полученные результаты.
...
### 4.5. Что означает «ускорить и улучшить legacy с помощью AMReX»

Legacy-код не переписывается задним числом и остаётся baseline. Улучшением
является новый AMReX solver, реализующий ту же физическую постановку с другой
пространственно-временной схемой и AMR. Гипотеза о превосходстве проверяется, а
не закладывается в критерий успеха.

Нужно построить Pareto-сравнение по ошибке, wall time и памяти. Отдельно
показать uniform AMReX, fixed AMR и dynamic AMR, чтобы не приписывать AMR
эффект MUSCL/RK2/CT или более удачной реализации. Отрицательный результат для
части задач является допустимым и должен быть объяснён.

### 4.6. Архитектурный задел для discontinuous Galerkin

Текущая фаза не реализует DG. Она обязана не закрыть путь к нему:

- отделить ideal-MHD physics/state/physical flux от FV reconstruction и CT;
- отделить spatial residual operator от time integrator;
- сохранить problem/BC, diagnostics, execution и I/O через явные интерфейсы;
- документировать зависимости ADR и защитить рефакторинг FV regression tests;
- не предполагать, что staggered CT автоматически подходит для DG: basis,
  quadrature, surface flux, limiter/positivity и divergence control получат
  отдельное ТЗ.
...
15. claim-to-evidence matrix для РПЗ НИРС;
16. ADR модульных границ `physics / spatial operator / time integrator /
    mesh backend / diagnostics` для будущего DG.
...
### WP-R0. Исправление текущей РПЗ

До крупных численных и performance-изменений провести аудит `docs/REPORT.md` и
`docs/report.tex`:

- составить claim-to-evidence matrix со статусами `implemented`, `verified`,
  `measured`, `hypothesis`, `planned`;
- убрать «гарантию div B = 0» из заголовка, если она не доказана для всех
  заявленных uniform/AMR режимов;
- явно написать, что gas reflux, полный GPU-порт и benchmark/scaling evidence
  пока отсутствуют;
- текущие времена обозначить диагностическими единичными запусками, а не
  benchmark;
- включить постановку H1 и заранее определить критерии её подтверждения/
  опровержения;
- оформить сокращённую РПЗ НИРС: цель, постановка, legacy, новый метод,
  методика проверки, имеющиеся результаты, ограничения, программа продолжения,
  выводы;
- обеспечить сборку PDF и отсутствие расхождений ключевых утверждений между
  Markdown и LaTeX.

Критерий приёмки: научному руководителю можно передать документ без claims,
которые сильнее имеющихся доказательств; каждая цифра имеет источник.
...
WP9 обновляет, а не заново изобретает draft WP-R0. Итоговая РПЗ должна разделять:
...
11. проверку H1 по Pareto-графикам error/time/memory;
12. архитектурный задел и отдельный roadmap DG без заявления о его реализации.
...
|---|---|---|
| G-R0 | исправленная РПЗ, claim-to-evidence matrix, собираемый PDF | текущий baseline |
...
| G8 | итоговые таблицы и финальная РПЗ | G-R0, G2–G7 |
...
- РПЗ различает реализованное, проверенное, измеренное, гипотезы и план;
- принят ADR границ FV/DG, а подготовительный рефакторинг защищён regression;
