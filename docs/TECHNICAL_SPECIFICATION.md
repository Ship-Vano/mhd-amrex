...
1. объяснить и количественно исследовать «немонотонность» в задаче Brio–Wu; исправить её адаптацией сетки, тк в старой версии программы таких результатов не было;
...
6. подготовить кластерные job scripts, сырые результаты, таблицы и итоговый
   научно-технический отчёт с стиле расчётно-пояснительной записки ВКР, но меньшего объёма (НИРС) для МГТУ им. Баумана.
