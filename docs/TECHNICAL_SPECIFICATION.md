# Техническое задание на завершение и исследование `mhd-amrex`
...
Довести текущий 2D/2.5D решатель идеальной магнитной гидродинамики до
воспроизводимого исследовательского программного комплекса, для которого:

1. объяснена и количественно исследована «немонотонность» в задаче Brio–Wu;
2. проведено прямое сравнение прежней и новой численных схем;
3. доказана корректность однородных и AMR-расчётов;
4. измерены время, strong/weak scaling, эффективность и ускорение CPU-кода;
5. реализован и проверен NVIDIA CUDA backend через AMReX;
6. подготовлены кластерные job scripts, сырые результаты, таблицы и итоговый
...
- Git-ветка `main` не имеет ни одного коммита; все файлы фактически untracked.
...
В конце работ в репозитории должны быть:

1. корректный Git baseline и `.gitignore`;
2. CTest suite для kernels, uniform grid, AMR, MPI и CUDA;
3. конфигурируемый verification/benchmark runner;
4. эталон и машинные метрики Brio–Wu;
5. harness сравнения legacy/new;
6. conservative AMR synchronization для gas + CT;
7. CPU timers и raw benchmark format;
8. strong/weak scaling scripts и таблицы;
9. CUDA build, GPU-порт и CPU/GPU parity tests;
10. Slurm scripts для CPU, hybrid и GPU запусков;
11. итоговые CSV/JSON, графики, таблицы Markdown/LaTeX;
12. обновлённый научно-технический отчёт без неподтверждённых утверждений.
...
- сделать начальный commit после ручной проверки владельцем;
