...
| AMR-001 | gas conservation across coarse/fine | `YAFluxRegister` на каждую пару уровней; CTest `amr.conservation` (падает и при отсутствии стыка уровней, и если при `reflux=false` дефект не проявляется). Статическая иерархия: дрейф ρ 6.9e-5 → ~6e-15, E 1.5e-4 → ~9e-16 | **implemented (T07)** | T07 |
