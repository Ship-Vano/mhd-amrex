...
| C-033 | Масштабирование на одном узле измерено (T11, частично) | **measured** | `docs/T09_TIMING.md` §7, `benchmarks/summary/scaling_{omp,mpi}_ot128.json`: OpenMP $S_4$=2.67 (пик на 4 производительных ядрах, затем спад), MPI $S_4$=3.42 ($E_4$=85 %). Многоузловое — блокировано D-005 |
...
| C-037 | SSP-свойство интегратора на Брио–Ву не даёт измеримого преимущества | **measured (отрицательный результат)** | SSP-RK2 (Хойн) и не-SSP средняя точка совпадают по relL1 до 4 значащих цифр и по выбросу до 0.1 %; TVD-RK3 при 1.5× стоимости не лучше. Первый порядок по времени удваивает выброс |
