...
| C-034 | Диапазоны ОТ у `legacy_corrected` согласуются с литературой с точностью, ожидаемой для первого порядка | **measured** | 128×128: ρ [0.0953, 0.4018], p [0.0417, 0.4454] против Авдеевой–Лукина (N=800) [0.087, 0.489] / [0.028, 0.490]. Структура (центральный ромб, симметричные уплотнения, диагональные фронты) воспроизводится; диапазоны сужены диффузией первого порядка при втрое меньшем разрешении |
